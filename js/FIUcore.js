...
var fiusubnav = {
    ...
    navOffsetTop: 0,
    init: function() {
        if (3 < $(".fiu-sub-nav > ul > li").length) {
            if ($(".fiu-sub-nav-wrapper, #fiu-sub-nav").removeClass("hide"),
            $("#more").width(76),
            this.hideItems(),
            $(window).on("resize", Foundation.utils.throttle(function(e) {
                fiusubnav.resize()
            }, 600)),
            $(".fiu-sub-nav-button").click(function() {
                $(".fiu-sub-nav > ul").slideToggle(250),
                $(".fiu-sub-nav-button .icon").toggleClass("close")
            }),
            $("#fiu-sub-nav").on("keydown", function(e) {
                "27" == e.keyCode && 0 < $("#fiu-sub-nav >nav >ul >li.open").length && (0 < $("#fiu-sub-nav a.active").length ? $("#fiu-sub-nav a.active").last() : $("#fiu-sub-nav >nav >ul >li.open")).focus().trigger("click")
            }),
            $(".fiu-sub-nav > ul > li > a.has-dropdown").click(function(e) {
                e.preventDefault(),
                e.stopImmediatePropagation(),
                $(this).parent().hasClass("open") ? $(this).parent().removeClass("open") : ($(".fiu-sub-nav .open").removeClass("open"),
                $(this).parent().addClass("open")),
                $("#fiu-sub-nav a").removeClass("active open")
            }),
            $("#fiu-sub-nav ul li ul li:last-child a").on("keydown", function(e) {
                "9" == e.keyCode && $(this).closest(".open").removeClass("open").prev().removeClass("active")
            }),
            $("#fiu-sub-nav ul").on("click", "ul a", function(e) {
                e.stopPropagation(),
            ...
            this.nav = document.getElementById("fiu-sub-nav"),
            this.navPosition = document.getElementById("fiu-sub-nav").offsetTop,
            this.navHeight = document.getElementById("fiu-sub-nav").offsetHeight - 16,
            "undefined" != typeof fiusubnavsticky && ($("#fiu-sub-nav").addClass("ready"),
            addEventListener("scroll", () => {
                fiusubnav.toggleSticky()
            }
            ),
            !document.querySelector(".fiu-sub-nav-wrapper"))) {
                let e = document.createElement("div");
                e.classList.add("fiu-sub-nav-wrapper"),
                this.nav.parentNode.insertBefore(e, this.nav),
                e.appendChild(this.nav)
            }
        } else
            $(".fiu-sub-nav-wrapper").removeClass("hide-for-large-up")
    },
    toggleSticky: function() {
        ...
        Foundation.utils.is_large_up() && (window.scrollY >= this.navPosition && "undefined" != typeof fiusubnavsticky ? (this.nav.classList.add("sticky"),
        $(document).foundation({
            "magellan-expedition": {
    ...
    hideItems: function() {
        this.calculate(),
        $("#fiu-sub-nav nav").removeClass("loaded"),
        Foundation.utils.is_large_up() ? ($(".fiu-sub-nav-wrapper, #fiu-sub-nav").removeClass("hide"),
        $("#fiu-sub-nav a").removeClass("active open"),
        $("#fiu-sub-nav > nav > ul").show(),
        $(".fiu-sub-nav > ul > li ").each(function() {
            fiusubnav.current += $(this).width(),
            fiusubnav.current >= fiusubnav.max && ($(this).attr("id") || $(this).addClass("hide"))
        }),
        this.showMore()) : $("#fiu-sub-nav > nav > ul ").hide(),
        $("#fiu-sub-nav nav").addClass("loaded")
    },
    calculate: function() {
        this.max = $(".fiu-sub-nav").width(),
        this.current = $("#more").width()
    },
    resize: function() {
        $("#more").width(76),
        $(".fiu-sub-nav-button .icon").removeClass("close"),
        $("#more").hide(),
        this.hideItems(),
        fiusubnav.toggleSticky()
    },
    showMore: function() {
        Foundation.utils.is_large_up() ? 1 <= $(".fiu-sub-nav > ul > li[class*=hide]").length && ($("#more ul").empty(),
        $(".fiu-sub-nav > ul > li[class*=hide]").each(function(e, a) {
            $(a).clone().appendTo("#more > ul").removeClass("hide")
        }),
        $("#more").show()) : $("#more").hide()
    },
    close: function() {
        $("#fiu-sub-nav .open").removeClass("open"),
        $("#fiu-sub-nav .active").removeClass("active")
    }
};
fiusubnav.init(),
